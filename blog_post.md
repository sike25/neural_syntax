...
We considered that raw accuracy metrics could be painting an incomplete picture. Consider this example: 
...
Notice how different rules can produce the same target subset. So we decided to compute a metric where a predicted rule is accurate if it produces the same subset of the world as the ground truth rule even if it is different from the ground truth rule. We called this the **adjusted accuracy** and calculated it at **43.24%**.
